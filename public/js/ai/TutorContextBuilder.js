...
const TutorContextBuilder = (function() {
    ...
    function buildContext(params) {
        ...
        const studentCtx = {
            ...
            mastery_profile: {
                topic: (profile.masteryByTopic || {})[qData.topic] || null,
                skills: (qData.skills || []).map(s => ({
                    skill_id: s,
                    level: (profile.masteryBySkill || {})[s] || null
                }))
            },
            recurring_errors: recurring,
            average_time_seconds: profile.averageTime || 0,
            cognitive_state: (typeof window !== 'undefined' && window.PUMKIN_DEEP_LEARNING_ENABLED && window.PumkinCognitiveTracker) ? window.PumkinCognitiveTracker.getStandardizedState() : null,
            deep_learning_active: (typeof window !== 'undefined') ? !!window.PUMKIN_DEEP_LEARNING_ENABLED : false
        };
